...
const DonationForm = dynamic(() => import("@/components/DonationForm"), {
  ssr: false,
  loading: () => (
    <div className="h-96 flex items-center justify-center text-black font-black animate-pulse uppercase tracking-widest text-xs">
...
function HeroSection() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative h-[85vh] flex flex-col items-center justify-center px-6 z-10 overflow-hidden"
    >
      <motion.div style={{ opacity, y }} className="text-center">
        ...
        <h1 className="text-7xl md:text-[11vw] font-black leading-[0.8] tracking-tighter uppercase mb-6">
          FAITHFUL <br />
          <span className="text-transparent bg-clip-text bg-linear-to-r from-[#F0C060] to-yellow-200">
            GIVING.
          </span>
        </h1>
        <p className="max-w-xl mx-auto text-white/40 text-lg md:text-xl font-light">
          Your generosity fuels our mission to spread the Gospel and serve the
          Lakeside community.
        </p>
      </motion.div>
    </section>
...
function PaymentStatusBanner() {
  ...
  if ((!success && !error) || !visible) return null;

  const isSuccess = success === "true";

  const errorMessages: Record<string, string> = {
    missing_reference: "Payment reference is missing. Please try again.",
    verification_failed: "We could not verify your payment. Please contact us.",
    update_failed: "Payment received but record update failed. Please contact us.",
    payment_failed: "Your payment was not completed. Please try again.",
    verification_error: "An unexpected error occurred during verification.",
  };
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 px-8 py-5 rounded-2xl shadow-2xl font-black text-sm uppercase tracking-widest max-w-md w-[90vw] text-center ${
        isSuccess ? "bg-[#F0C060] text-[#0F0C1E]" : "bg-red-500 text-white"
      }`}
    >
      {/* Close button */}
      <button
      ...
      {isSuccess ? (
        <span>
          🙏 Blessing received! Your gift has been recorded.
          {reference && (
            <span className="block text-[10px] mt-1 opacity-60 normal-case tracking-wide">
              Ref: {reference}
            </span>
          )}
        </span>
...
export default function DonatePageClient() {
  ...
  return (
    <main className="relative min-h-screen bg-[#06040d] text-white overflow-x-hidden selection:bg-[#F0C060] selection:text-black">
      ...
      {/* Banner reads URL params — safe inside Suspense from page.tsx */}
      <PaymentStatusBanner />
        ...
        <div className="max-w-7xl mx-auto px-6 pt-10">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <motion.div
              ...
              className="space-y-8"
            >
              <h4 className="text-[#F0C060] font-black uppercase tracking-[0.3em] text-xs">
                Our Purpose
              </h4>
              <h2 className="text-5xl md:text-7xl font-black leading-none uppercase">
                YOUR GIFT, <br />
                HIS <span className="italic text-[#F0C060]">GLORY.</span>
              </h2>
              <p className="text-xl text-black/60 leading-relaxed font-medium">
                At Lakeside Baptist Church, we believe giving is an act of
                worship. Your contributions support local outreaches, global
                missions, and the nurturing of a Christ-centered community.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-6 mt-40">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              {
                t: "Tithes",
                d: "The first tenth of our increase, returned to God in gratitude.",
                i: "⛪",
              },
              {
                t: "Offerings",
                d: "Freewill gifts above the tithe to support special projects.",
                i: "🎁",
              },
              {
                t: "Missions",
                d: "Direct support for our missionaries spreading the Gospel globally.",
                i: "🌍",
              },
            ].map((card, idx) => (
              <motion.div
